...
__all__ = ['ResNet', 'svhn_resnet18', 'svhn_resnet18_bwn_shift', 'svhn_resnet18_bwn_all']
...
# all layers are quantized below
...
def svhn_resnet18(pretrained=False, **kwargs):

    model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=10)
    if pretrained:
        print("load!")
        info = torch.load("../scripts/logger/svhn_resnet18_baseline/svhn_resnet18_best.pth.tar")
        model.load_state_dict(info['state_dict'], strict=False)

    return model


def svhn_resnet18_bwn_all(pretrained=False, **kwargs):
...
def svhn_resnet18_bwn_shift(pretrained=True, **kwargs):
